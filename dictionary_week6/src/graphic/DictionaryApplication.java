...
public class DictionaryApplication {
	...
	public void intalize() {
		...
		// Danh sách các button library, refresh, search, add, edit, delete, speech
		...
		library.setLocation(378, 20);
		
		
		refresh = new JButton();
		refresh.setIcon(new ImageIcon(DictionaryApplication.class.getResource("/icon/icons8_restore_32px_1.png")));
		...
		refresh.setBounds(441, 499, 42, 30);
		...
		speech = new JButton();
		speech.setIcon(new ImageIcon(DictionaryApplication.class.getResource("/icon/icons8_sound_32px.png")));
		speech.setBackground(null);
		speech.setFont(new Font("Tw Cen MT", Font.BOLD, 15));
		speech.setForeground(Color.RED);
		speech.setBorder(null);
		speech.setBounds(441, 13, 42, 45);
		
		
		
		api = new JButton();
		api.setIcon(new ImageIcon(DictionaryApplication.class.getResource("/icon/icons8_rest_api_32px.png")));
		api.setBackground(null);
		api.setBorder(null);
		api.setBounds(389, 499, 42, 30);
		
		
		// Add tất cả các elements trên vào giao diện home
		
		home.getContentPane().add(api);
		home.getContentPane().add(searchBox);
		home.getContentPane().add(speech);
...
//					searchWord = searchWord.replace(searchWord.charAt(0), Character.toUpperCase(searchWord.charAt(0)));
		...
		//Click vô button "Search" để kiếm từ trong hộp search
...
//				searchWord = searchWord.replace(searchWord.charAt(0), Character.toUpperCase(searchWord.charAt(0)));
		...
		delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String wordPoint = null;
				if ( listWord.getSelectedIndex()!= -1 ) wordPoint = listWord.getSelectedValue();
				new FrameDeleteWord(admin, wordPoint);
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ( listWord.getSelectedIndex()!=-1 ) {
					...
					try {
						...
						voice.setPitch(150);
						voice.setVolume(8);
						voice.speak(listWord.getSelectedValue());
					}
					catch (Exception e1) {
						e1.printStackTrace();
					}
