...
public class FrameDeleteWord {
	...
	private void action(DictionaryCommmandLine admin, Vector<String> words, String wordPoint) {
		
		Vector<String> listDelete = new Vector<>();
		
		if (wordPoint!=null) {
			listDelete.add(wordPoint);
	...
	public FrameDeleteWord(DictionaryCommmandLine admin, String wordPoint) {
		
		
		
		this.intinalize(admin, wordPoint);
		
	}
	
	
	private void intinalize(DictionaryCommmandLine admin, String wordPoint) {
		
		Vector<String> words = admin.getAllWordTarget();
		
		if (wordPoint!=null) words.remove(admin.getManagement().getDictionary().indexOfWordTarget(wordPoint));
		...
		this.action(admin, words, wordPoint);
	}
}
